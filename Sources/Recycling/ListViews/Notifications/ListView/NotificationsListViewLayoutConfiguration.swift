...
enum NotificationsListViewLayoutConfiguration {
    case small
    case medium
    case large
    ...
    init(width: CGFloat) {
        switch width {
        case let width where width > NotificationsListViewLayoutConfiguration.mediumRange.upperBound: self = .large
        case let width where width < NotificationsListViewLayoutConfiguration.mediumRange.lowerBound: self = .small
        default: self = .medium
        }
    }
