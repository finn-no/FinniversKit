...
protocol NotificationsListViewLayoutDelegate {
...
class NotificationsListViewLayout: UICollectionViewLayout {
    private let delegate: NotificationsListViewLayoutDelegate
    private var itemAttributes = [UICollectionViewLayoutAttributes]()

    private var configuration: NotificationsListViewLayoutConfiguration {
        guard let collectionView = collectionView else {
            fatalError("Layout unusable without collection view!")
        }

        return NotificationsListViewLayoutConfiguration(width: collectionView.frame.size.width)
    }

    init(delegate: NotificationsListViewLayoutDelegate) {
    ...
    override func prepare() {
        ...
        if let height = delegate.notificationsListViewLayout(self, heightForHeaderViewInCollectionView: collectionView) {
        ...
        for index in 0 ..< numberOfItems {
            ...
            let imageHeightRatio = delegate.notificationsListViewLayout(self, imageHeightRatioForItemAtIndexPath: indexPath, inCollectionView: collectionView)
            let itemNonImageHeight = delegate.notificationsListViewLayout(self, itemNonImageHeightForItemAtIndexPath: indexPath, inCollectionView: collectionView)
